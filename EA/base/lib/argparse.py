__version__ = '1.1'__all__ = ['ArgumentParser', 'ArgumentError', 'ArgumentTypeError', 'FileType', 'HelpFormatter', 'ArgumentDefaultsHelpFormatter', 'RawDescriptionHelpFormatter', 'RawTextHelpFormatter', 'MetavarTypeHelpFormatter', 'Namespace', 'Action', 'ONE_OR_MORE', 'OPTIONAL', 'PARSER', 'REMAINDER', 'SUPPRESS', 'ZERO_OR_MORE']import os as _osimport re as _reimport sys as _sysfrom gettext import gettext as _, ngettextSUPPRESS = '==SUPPRESS=='OPTIONAL = '?'ZERO_OR_MORE = '*'ONE_OR_MORE = '+'PARSER = 'A...'REMAINDER = '...'_UNRECOGNIZED_ARGS_ATTR = '_unrecognized_args'
...
class _ActionsContainer(object):
    ...
    def add_argument(self, *args, **kwargs):
        ...
        if args and len(args) == 1 and args[0][0] not in chars:
    ...
    def _add_action(self, action):
        ...
        for option_string in action.option_strings:
            if self._negative_number_matcher.match(option_string) and not self._has_negative_number_optionals:
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def parse_known_args(self, args=None, namespace=None):
        ...
        if namespace is None:
            namespace = Namespace()
        for action in self._actions:
            if action.dest is not SUPPRESS and (hasattr(namespace, action.dest) or action.default is not SUPPRESS):
    ...
    def _read_args_from_files(self, arg_strings):
        ...
        for arg_string in arg_strings:
            if arg_string and arg_string[0] not in self.fromfile_prefix_chars:
    ...
    def _match_argument(self, action, arg_strings_pattern):
        ...
        if match is None:
            nargs_errors = {ONE_OR_MORE: _('expected at least one argument'), OPTIONAL: _('expected at most one argument'), None: _('expected one argument')}
    ...
    def _get_values(self, action, arg_strings):
        ...
        elif arg_strings or not (action.nargs == ZERO_OR_MORE and action.option_strings):
