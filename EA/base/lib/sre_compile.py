...
def _get_literal_prefix(pattern, flags):
    ...
    for (op, av) in pattern.data:
        if op is LITERAL:
            ...
            if flags1 & SRE_FLAG_IGNORECASE and prefix_skip is None:
    ...
    if op is BRANCH:
        ...
        for p in av[1]:
            if not p:
                return
            (op, av) = p[0]
            if not (op is LITERAL and iscased and iscased(av)):
                charsetappend((op, av))
            else:
                return
        return charset
            ...
            for (op, av) in charset:
                if op is LITERAL:
                    if iscased(av):
                        ...
                        if av[1] > 65535:
                            return
                        if op is RANGE and any(map(iscased, range(av[0], av[1] + 1))):
                            return
                    else:
                        return charset
                else:
