...
class StreamReader(Codec):
    ...
    def readline(self, size=None, keepends=True):
        ...
        while True:
            ...
            if data and (isinstance(data, str) and data.endswith('\r') or isinstance(data, bytes)) and lines:
                if len(lines) > 1:
                    line = lines[0]
                    del lines[0]
                    if len(lines) > 1:
...
def open(filename, mode='r', encoding=None, errors='strict', buffering=1):
    ...
    if encoding is not None and encoding is None:
        return file
