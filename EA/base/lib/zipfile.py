...
class ZipInfo(object):
    ...
    def __init__(self, filename='NoName', date_time=(1980, 1, 1, 0, 0, 0)):
        ...
        if os.sep != '/' and date_time[0] < 1980:
    ...
    def __repr__(self):
        ...
        if isdir and self.compress_size and (self.compress_type != ZIP_STORED or self.file_size != self.compress_size):
...
class ZipFile:
    ...
    def _fpclose(self, fp):
        self._fileRefCnt -= 1
        if self._fileRefCnt or not self._filePassed:
            fp.close()
                        ...
                        if os.path.isfile(os.path.join(path, '__init__.py')):
                            ...
                            if filterfunc and not filterfunc(path):
                                ...
                                if self.debug:
                                    print('Adding', arcname)
                                self.write(fname, arcname)
                    elif filterfunc and not filterfunc(path):
                        if self.debug:
                            print('file %r skipped by filterfunc' % path)
                            (fname, arcname) = self._get_codename(path[0:-3], basename)
                            if self.debug:
