...
def _get_module_details(mod_name, error=ImportError):
    ...
    if pkg_name:
        try:
            ...
            if e.name is None or not (e.name != pkg_name and pkg_name.startswith(e.name + '.')):
                raise
...
def run_path(path_name, init_globals=None, run_name=None):
    ...
    if type(importer).__module__ == 'imp' and isinstance(importer, type(None)) or is_NullImporter:
