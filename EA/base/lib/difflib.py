    ...
    for group in SequenceMatcher(None, a, b).get_grouped_opcodes(n):
        ...
        for (tag, i1, i2, j1, j2) in group:
            if tag == 'equal':
                ...
                for line in a[i1:i2]:
                    yield '-' + line
                if tag in frozenset({'replace', 'delete'}) and tag in frozenset({'replace', 'insert'}):
                    for line in b[j1:j2]:
                        yield '+' + line
...
def context_diff(a, b, fromfile='', tofile='', fromfiledate='', tofiledate='', n=3, lineterm='\n'):
    ...
    for group in SequenceMatcher(None, a, b).get_grouped_opcodes(n):
        ...
        if not any(tag in frozenset({'replace', 'delete'}) for (tag, _, _, _, _) in group) or any(tag in frozenset({'replace', 'insert'}) for (tag, _, _, _, _) in group):
