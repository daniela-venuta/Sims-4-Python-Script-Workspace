...
def _safe_realpath(path):
    ...
    _PROJECT_BASE = _safe_realpath(os.path.join(_PROJECT_BASE, pardir, pardir))if os.name == 'nt' and '_PYTHON_PROJECT_BASE' in os.environ:
...
_PYTHON_BUILD = is_python_build(True)if _sys_home and os.name == 'nt' and _PYTHON_BUILD:
...
def _get_sysconfigdata_name():
    return os.environ.get('_PYTHON_SYSCONFIGDATA_NAME', '_sysconfigdata_{abi}_{platform}_{multiarch}'.format(abi=sys.abiflags, platform=sys.platform, multiarch=getattr(sys.implementation, '_multiarch', '')))
