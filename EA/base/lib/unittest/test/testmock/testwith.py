...
class WithTest(unittest.TestCase):
    ...
    def test_with_statement_nested(self):
        with catch_warnings(record=True), patch('%s.something' % __name__) as mock_something, patch('%s.something_else' % __name__) as mock_something_else:
            self.assertEqual(something, mock_something, 'unpatched')
...
class TestMockOpen(unittest.TestCase):
    ...
    def test_read_bytes(self):
        ...
        with patch('%s.open' % __name__, mock, create=True), open('abc', 'rb') as f:
            result = f.read()
        self.assertEqual(result, b'\xc6')

    def test_readline_bytes(self):
        m = mock_open(read_data=b'abc\ndef\nghi\n')
        with patch('%s.open' % __name__, m, create=True), open('abc', 'rb') as f:
            line1 = f.readline()
            line2 = f.readline()
            line3 = f.readline()
