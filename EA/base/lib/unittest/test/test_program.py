...
class Test_TestProgram(unittest.TestCase):
    ...
    def test_NonExit(self):
        program = unittest.main(exit=False, argv=['foobar'], testRunner=unittest.TextTestRunner(stream=io.StringIO()), testLoader=self.FooBarLoader())
        self.assertTrue(hasattr(program, 'result'))

    def test_Exit(self):
        self.assertRaises(SystemExit, unittest.main, argv=['foobar'], testRunner=unittest.TextTestRunner(stream=io.StringIO()), exit=True, testLoader=self.FooBarLoader())

    def test_ExitAsDefault(self):
        self.assertRaises(SystemExit, unittest.main, argv=['foobar'], testRunner=unittest.TextTestRunner(stream=io.StringIO()), testLoader=self.FooBarLoader())
...
class TestCommandLineArgs(unittest.TestCase):
    ...
    def testSelectedTestNamesFunctionalTest(self):
        ...
        def run_unittest(args):
            p = subprocess.Popen([sys.executable, '-m', 'unittest'] + args, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, cwd=os.path.dirname(__file__))
            with p:
