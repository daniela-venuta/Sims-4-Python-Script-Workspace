...
class Decimal(object):
        ...
        if isinstance(value, (list, tuple)):
            ...
            if value[2] == 'F':
                ...
                for digit in value[1]:
                    if isinstance(digit, int) and 0 <= digit and digit <= 9:
    ...
    def __add__(self, other, context=None):
        ...
        if self._sign != other._sign:
            negativezero = 1
        if context.rounding == ROUND_FLOOR and self or not other:
    ...
    def _divide(self, other, context):
        ...
        if self and (other._isinfinity() or expdiff <= -2):
    ...
    def _power_exact(self, other, p):
        ...
        if ye >= 0:
            ...
            n = 10**(-ye)
            while True:
                while m % 2 == n % 2 and n % 2 == 0:
                    m //= 2
                    n //= 2
            while True:
                while m % 5 == n % 5 and n % 5 == 0:
                    m //= 5
                    n //= 5
        if n > 1:
    ...
    def quantize(self, exp, rounding=None, context=None):
        ...
        if not (context.Etiny() <= exp._exp and exp._exp <= context.Emax):
    ...
    def logical_and(self, other, context=None):
        if context is None:
            context = getcontext()
        other = _convert_other(other, raiseit=True)
        if not (self._islogical() and other._islogical()):
    ...
    def rotate(self, other, context=None):
        ...
        if other._exp != 0:
            return context._raise_error(InvalidOperation)
        if not (-context.prec <= int(other) and int(other) <= context.prec):
            return context._raise_error(InvalidOperation)
        if self._isinfinity():
            return Decimal(self)
        torot = int(other)
    ...
    def scaleb(self, other, context=None):
        ...
        if not (liminf <= int(other) and int(other) <= limsup):
            return context._raise_error(InvalidOperation)
        if self._isinfinity():
            return Decimal(self)
    ...
    def __format__(self, specifier, context=None, _localeconv=None):
        ...
        if precision is not None:
            ...
            elif len(self._int) > precision:
                self = self._round(precision, rounding)
        if spec['type'] in 'fF%':
            self = self._rescale(0, rounding)
        leftdigits = self._exp + len(self._int)
        if self or self._exp > 0 and spec['type'] in 'eE':
            if self or precision is not None:
...
def _convert_for_comparison(self, other, equality_op=False):
    ...
    if other.imag == 0:
        other = other.real
    if equality_op and isinstance(other, _numbers.Complex) and isinstance(other, float):
        context = getcontext()
        if equality_op:
...
def _parse_format_specifier(format_spec, _localeconv=None):
    ...
    if format_dict['precision'] == 0 and format_dict['type'] == 'n':
        format_dict['type'] = 'g'
