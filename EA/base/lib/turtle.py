...
class TurtleScreenBase(object):
    ...
    def _onclick(self, item, fun, num=1, add=None):
        if fun is None:
            ...
            def eventfun(event):
                x = self.cv.canvasx(event.x)/self.xscale
                y = -self.cv.canvasy(event.y)/self.yscale
                fun(x, y)
    ...
    def _resize(self, canvwidth=None, canvheight=None, bg=None):
        ...
        if canvwidth is canvheight and canvheight is bg and bg is None:
...
class TPen(object):
    ...
    def pen(self, pen=None, **pendict):
        ...
        if self._pensize != p['pensize']:
            newLine = True
        if 'pendown' in p and 'pencolor' in p and 'pensize' in p and newLine:
            self._newLine()
        if 'pendown' in p:
        ...
        if 'stretchfactor' in p or 'tilt' in p or 'shearfactor' in p:
            ...
            sa = math.sin(self._tilt)
            ca = math.cos(self._tilt)
...
class _TurtleImage(object):
    ...
    def _setshape(self, shapeIndex):
        ...
        if self._type == 'polygon' and 'polygon' == screen._shapes[shapeIndex]._type:
            return
        if self._type == 'image' and 'image' == screen._shapes[shapeIndex]._type:
            return
...
class RawTurtle(TPen, TNavigator):
    ...
    def shapesize(self, stretch_wid=None, stretch_len=None, outline=None):
        if stretch_wid is stretch_len and stretch_len is outline and outline is None:
    ...
    def shapetransform(self, t11=None, t12=None, t21=None, t22=None):
        if t11 is t12 and (t12 is t21 and t21 is t22) and t22 is None:
