...
def pprint(object, stream=None, indent=1, width=80, depth=None, *, compact=False):
    printer = PrettyPrinter(stream=stream, indent=indent, width=width, depth=depth, compact=compact)
        ...
        for (i, line) in enumerate(lines):
            ...
            if len(rep) <= max_width1:
                ...
                for (j, part) in enumerate(parts):
                    ...
                    if j == len(parts) - 1 and len(repr(candidate)) > max_width2:
                        if current:
...
def _wrap_bytes_repr(object, width, allowance):
    ...
    for i in range(0, len(object), 4):
        ...
        candidate = current + part
        width -= allowance
        if not i == last or len(repr(candidate)) > width:
            if current:
                yield repr(current)
            current = part
        else:
