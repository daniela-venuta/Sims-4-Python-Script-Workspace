...
class StartCraftingSuperInteraction(StartCraftingMixin, PickerSuperInteraction):
    ...
    def picker_rows_gen(cls, inst, target, context, crafter=DEFAULT, order_count=1, recipe_ingredients_map=None, funds_source=None, **kwargs):
        ...
        for recipe in recipe_list:
            ...
            if (not recipe.use_ingredients is not None or ingredients_needed_count and subclass_of_order_interaction) and (cls.ingredient_source and has_required_ingredients or funds_source is None) and recipe_test_result.visible:
...
class StartCraftingOrderHandler:
    ...
    def place_order_for_recipe(self, recipe):
        ...
        elif self._start_crafting_si._handle_begin_crafting(recipe, self._crafter, ordering_sim=self._orderer, orderer_ids=self._start_crafting_si.orderer_ids, funds_source=self._funds_source):
            crafting_si = self.get_existing_order(recipe)
            self._process = crafting_si.process
            result = self.push_wait_for_order(crafting_si)
        if not result:
