...
class CancelAOPLiability(Liability):
    ...
    def release(self):
        ...
        sim = self._sim_ref()
        posture = sim.posture
        if sim is not None and posture is self._posture and (posture.is_vehicle or posture.ownable and (self._posture.owning_interactions and len(self._posture.owning_interactions) == 1) and self._interaction_cancel_replacement_ref() in self._posture.owning_interactions):
