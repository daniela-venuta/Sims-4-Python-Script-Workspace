...
class PickerSingleChoiceSuperInteraction(PickerSuperInteraction):
    ...
    def _show_picker_dialog(self, owner, target_sim=None, target=None, choices=(), **kwargs):
        ...
        if len(choices) == 1 and picked_items:
    ...
    def _setup_dialog(self, dialog, **kwargs):
        ...
        for ((definition, custom_price), objects) in purchase_picker_data.items_to_purchase.items():
            if multiplier != 1:
                ...
                tooltip = None
            row = PurchasePickerRow(def_id=definition.id, is_enable=self._get_enabled_option(definition), num_owned=self._get_count_option(definition), num_available=self._get_availability_option(definition), tags=definition.build_buy_tags, custom_price=custom_price, objects=objects, row_tooltip=tooltip, show_discount=self.show_discount, icon_override=self._get_icon_override_option(definition), row_description=self._get_description_override_option(definition))
            dialog.add_row(row)
    ...
    def _setup_dialog(self, dialog, **kwargs):
        ...
        for ((definition, custom_price), objects) in purchase_picker_data.items_to_purchase.items():
            ...
            if multiplier != 1:
                ...
                custom_price = int(original_price*multiplier)
                tooltip = None
            row = PurchasePickerRow(def_id=definition.id, num_available=count, custom_price=custom_price, tags=definition.build_buy_tags, objects=objects, row_tooltip=tooltip, show_discount=self.show_discount)
            dialog.add_row(row)
