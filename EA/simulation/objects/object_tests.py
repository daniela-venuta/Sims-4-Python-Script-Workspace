    ...
    def __call__(self, inventory_owners=None, content_check_participant=None):
        ...
        for household in households:
            ...
            if not self.required_count.compare(count):
                return TestResultNumeric(False, '{} Household Inventory does not have required number of objects in it', household, tooltip=self.tooltip, current_value=count, goal_value=self.required_count.value, is_money=False)
        ...
        for target in targets:
            ...
            if parent_objects is None:
                if self.negate:
                    ...
                    if parent is not None and self.require_parent_is_surface and parent.is_surface():
                ...
                if not test_parent.is_surface():
                    test_parent = None
                if self.require_parent_is_surface and self.negate:
