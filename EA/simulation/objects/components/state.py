...
class _ModifierValueWithMultipliers(HasTunableSingletonFactory, AutoFactoryInit):
...
class _BasicModifierValue(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'value': Tunable(description='\n            A basic float that will be added to the modifier.\n            ', tunable_type=float, default=0)}

    def __call__(self, obj):
        return self.value

class _TileCountModifierValue(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def set_state(self, state, new_value, from_stat=False, from_init=False, from_load=False, immediate=False, force_update=False, from_sync=False, **kwargs):
        ...
        if state not in self._states:
            if new_value.force_add_state:
                ...
                logger.warn("Attempting to set the value of the '{}' state on object {}, but the object's definition ({}) isn't tuned to have that state, and force_add_state is false.", state_name, self.owner, self.owner.definition.name)
                return
