...
class LightingComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=LIGHTING_COMPONENT, persistence_key=protocols.PersistenceMaster.PersistableData.LightingComponent):
    ...
    def is_power_off(self):
        if services.utilities_manager().is_utility_active(Utilities.POWER) or not get_object_has_tag(self.owner.definition.id, LightingComponent.NON_ELECTRIC_LIGHT_TAG):
            return True
        return False

    def update_lighting_enabled_state(self):
        if self.is_power_off() and get_object_has_tag(self.owner.definition.id, LightingComponent.NON_ELECTRIC_LIGHT_TAG) and (self.disabling_state_values and any(self.owner.state_value_active(state_value) for state_value in self.disabling_state_values) or self._disable_because_sold):
