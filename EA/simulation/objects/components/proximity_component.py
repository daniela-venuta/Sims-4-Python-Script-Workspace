...
class ProximityComponent(objects.components.Component, sims4.tuning.tunable.HasTunableFactory, component_name=objects.components.types.PROXIMITY_COMPONENT, allow_dynamic=True):
    ...
    def _get_nearby_sims(self):
        ...
        nearby_sims = placement.get_nearby_sims_gen(position=self.owner.position, surface_id=self.owner.routing_surface, radius=self.update_distance, exclude=exclude, stop_at_first_result=False, only_sim_position=True, check_all_surfaces_on_level=True)
