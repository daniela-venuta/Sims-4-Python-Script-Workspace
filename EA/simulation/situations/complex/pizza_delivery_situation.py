...
class PizzaDeliverySituation(SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(2, _RingDoorBellState), SituationStateData(3, _NoFrontDoorState), SituationStateData(4, _WaitForCustomerState), SituationStateData(5, WaitForTipState, factory=cls.wait_for_tip_state), SituationStateData(6, _LeaveState), SituationStateData(7, _DeliveryFailureState))
    ...
    def _get_role_state_overrides(self, sim, job_type, role_state_type, role_affordance_target):
        if self.owner._crafted_object_id != 0:
            ...
            if obj_def_to_craft == 0:
                ...
                if recipe is None:
                    raise ValueError('No recipe for {}'.format(self))
            target = create_craftable(recipe, self.owner._service_npc, owning_household_id_override=self.owner._hiring_household.id, place_in_inventory=True)
            if target is None:
