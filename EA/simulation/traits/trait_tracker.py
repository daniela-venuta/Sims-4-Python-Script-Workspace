...
class TraitTracker(AffordanceCacheMixin, SimInfoTracker):
    ...
    def can_add_trait(self, trait):
        ...
        if self.is_conflicting(trait):
            ...
            return False
        elif trait.trait_type == TraitType.LIFESTYLE and not services.lifestyle_service().lifestyles_enabled:
            return False
        return True
    ...
    def picker_rows_gen(cls, inst, target, context, **kwargs):
        ...
        for trait in cls._trait_selection_gen(trait_target):
            if trait.display_name:
                ...
                row = ObjectPickerRow(name=display_name, row_description=trait.trait_description(trait_target), icon=trait.icon, tag=trait, is_enable=is_enabled, row_tooltip=row_tooltip)
                yield row
