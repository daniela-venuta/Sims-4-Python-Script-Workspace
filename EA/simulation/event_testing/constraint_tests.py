            ...
            if target.is_sim:
                ...
                if target is None:
                    ...
                    for sim in instanced_sims:
                        if not (total_constraint.geometry.test_transform(sim.transform) and (total_constraint.is_routing_surface_valid(sim.routing_surface) and (total_constraint.is_location_water_depth_valid(sim.location) and total_constraint.is_location_terrain_tags_valid(sim.location))) and (self.test_set(DoubleSimResolver(test_actor, sim.sim_info)) or self.must_be_line_of_sight and sim.sim_info is test_actor)):
