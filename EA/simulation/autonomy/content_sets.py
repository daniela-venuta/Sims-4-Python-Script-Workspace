...
def get_valid_aops_gen(target, affordance, si_affordance, si, context, include_failed_aops_with_tooltip, push_super_on_prepare=False, considered=None, aop_kwargs=frozendict(), aop_pre_tests=None):
    ...
    for aop in potential_interactions:
        test_result = aop_valid_for_scoring(aop, affordance, target, context, include_failed_aops_with_tooltip, considered=considered, aop_pre_tests=aop_pre_tests)
                                ...
                                if targets is None:
                                    ...
                                    if affordance.test_globals.run_tests(resolver, skip_safe_tests=False, search_for_tooltip=False):
                                        if not affordance.tests.run_tests(resolver, skip_safe_tests=False, search_for_tooltip=False):
                                            ...
                                            for target in targets:
                                                valid_aops_gen = get_valid_aops_gen(target, affordance, super_affordance, super_interaction, context, include_failed_aops_with_tooltip, push_super_on_prepare=push_super_on_prepare, considered=gsi_considered, aop_kwargs=aop_kwargs, aop_pre_tests=aop_pre_tests)
...
def get_buff_aops(sim, buff, super_interaction, context, potential_targets=(), gsi_considered=None, aop_pre_tests=None):
    ...
    for buff_affordance in buff.interactions.interaction_items:
        ...
        if targets is None:
            ...
            for target in targets:
                for (aop, test_result) in get_valid_aops_gen(target, buff_affordance, super_interaction.super_affordance, super_interaction, context, False, considered=gsi_considered, aop_pre_tests=aop_pre_tests):
