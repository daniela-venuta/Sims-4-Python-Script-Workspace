...
def test_criminal_generation(opt_sim:OptionalSimInfoParam=None, _connection=None):
    ...
    def try_generate_criminal(chosen_clues):
        try:
            ...
            filter_results = services.sim_filter_service().submit_matching_filter(sim_filter=criminal_filter, requesting_sim_info=sim_info, allow_yielding=False, gsi_source_fn=lambda : 'DetectiveCommands: Generate criminal based on {}'.format(str(chosen_clues)))
