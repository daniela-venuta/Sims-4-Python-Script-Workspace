...
def cast(t, *args, **kwargs):
    ...
    if hasattr(t, '__call__') and isinstance(t, type) and not (len(args) != 1 or args[0] is not None and isinstance(args[0], t)):
        return t(*args)
    else:
        return args[0]
...
def _specialized_from_args(signatures, args, kwargs):
    ...
    py_long = typedef(int, 'long')py_float = typedef(float, 'float')py_complex = typedef(complex, 'double complex')int_types = ['char', 'short', 'Py_UNICODE', 'int', 'Py_UCS4', 'long', 'longlong', 'Py_ssize_t', 'size_t']float_types = ['longdouble', 'double', 'float']complex_types = ['longdoublecomplex', 'doublecomplex', 'floatcomplex', 'complex']other_types = ['bint', 'void', 'Py_tss_t']to_repr = {'longlong': 'long long', 'longdouble': 'long double', 'longdoublecomplex': 'long double complex', 'doublecomplex': 'double complex', 'floatcomplex': 'float complex'}.getgs = globals()try:
