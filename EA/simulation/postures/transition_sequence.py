...
class TransitionSequenceController:
        ...
        for (posture_spec_template, posture_spec_vars, constraint) in posture_specs_and_vars:
            if any(isinstance(v, PostureSpecVariable) for v in posture_spec_vars.values()):
                ...
                if interaction is not None:
                    ...
                    if interaction.posture_preferences is not None and (interaction.posture_preferences.prefer_clicked_part and pick is not None) and pick.target is not None:
    ...
    def _create_transition_multi_exit(self, sim, sim_current_state, sim_edge):
        ...
        def do_transition_multi_exit(timeline):
            ...
            if linked_target_posture._primitive is None and (self.interaction is None or self.interaction.should_push_posture_primitive_for_multi_exit()):
    ...
    def _handle_vehicle_transition_info(self, sim, actor_transitions, current_state, next_state):
        ...
        if is_vehicle_posture_change or (vehicle_transition_state == VehicleTransitionState.DEPLOYING or vehicle_transition_state == VehicleTransitionState.MOUNTING) or len(sim.posture_state.get_free_hands()) == 2:
            path_spec = self._get_path_spec(sim)
