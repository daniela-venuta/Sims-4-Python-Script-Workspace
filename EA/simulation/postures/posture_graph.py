    ...
    def generate_transition_interactions(self, sim, final_si, transition_success):
        ...
        if not preload_outfit_set:
            ...
            if current_position_on_active_lot and not sim.is_outside:
                ...
                if sim.intended_position_on_active_lot and build_buy.is_location_outside(sim.intended_position, level):
...
class PostureGraphService(Service):
    ...
    def _validate_node_for_graph(node):
        ...
        if target is not None and not node[BODY_INDEX][BODY_POSTURE_TYPE_INDEX].mobile:
            surface = node[SURFACE_INDEX]
            if surface is None or surface[SURFACE_TARGET_INDEX] is None:
                ...
                if target_parent is not None and not target.is_set_as_head:
                    ...
                    if parent_slot and parent_slot.slot_types and target.slot_type_set:
                ...
                target_parent = target.parent
                if target_parent and surface.target == target_parent:
                    parent_slot = target.parent_slot
                    if parent_slot is None or not (parent_slot.slot_types and target.slot_type_set):
                        return False
                    else:
                        possible_slot_types = parent_slot.slot_types.intersection(target.slot_type_set.slot_types)
                        if not any(slot_type.implies_slotted_object_has_surface for slot_type in possible_slot_types):
    ...
    def _left_path_gen(self, sim, source, destinations, interaction, constraint, var_map, valid_edge_test, is_complete):
        ...
        if is_complete:
            if source[BODY_INDEX][BODY_POSTURE_TYPE_INDEX].mobile and source[SURFACE_INDEX][SURFACE_TARGET_INDEX] is None and (source[BODY_INDEX][BODY_POSTURE_TYPE_INDEX].is_vehicle or all(dest[BODY_INDEX][BODY_TARGET_INDEX] != source[BODY_INDEX][BODY_TARGET_INDEX] for dest in destinations)):
                return
    ...
    def get_segmented_paths(self, sim, posture_dest_list, additional_template_list, interaction, participant_type, valid_destination_test, valid_edge_test, preferences, final_constraint, included_sis):
        ...
        relationship_bonuses = PostureScoring.build_relationship_bonuses(sim, interaction.sim_affinity_posture_scoring_data, interaction.sim_affinity_use_current_position_for_none, sims_to_consider=interaction_sims)
        ...
        for (constraint, templates) in posture_dest_list.items():
            ...
            if source_spec is None:
                ...
                if source_spec.body_posture.mobile and (source_spec.body_posture.unconstrained and source_spec.body_target is not None) and (source_spec.body_target.is_part or source_spec not in self._graph):
                ...
                for node in self._graph.get_matching_nodes_gen(destination_specs, slot_types, constraint=matching_node_constraint):
                    if node.get_core_objects() & excluded_objects:
                        ...
                        pass
                    elif interaction.context.source == InteractionContext.SOURCE_AUTONOMY and interaction.autonomy_preference is not None and node.body_target is not None:
                        ...
                        if preference_target.is_part and preference_type == AutonomyPreferenceType.DISALLOWED:
                            pass
                        else:
                ...
                if interaction.is_putdown and destination_nodes:
                                ...
                                for goal in routing_goals:
                                    if single_goal_only or not (goal.requires_los_check and target is not None and (target.is_sim or ignore_los_for_vehicle)):
                                        if goal.failure_reason != GoalFailureType.NoError and blocking_obj_id is not None:
                                            ...
                                            (result, blocking_obj_id) = target.check_line_of_sight(goal.location.transform, verbose=True, for_carryable=for_carryable, use_standard_ignored_objects=True)
            ...
            for (destination_handle, (right_path, path_cost, var_map, dest_spec, _, _, _)) in destination_handles.items():
                ...
                for dest_handle in additional_dest_handles:
                    ...
                    if interaction.carry_target is dest_handle.target:
                        height_limit = None
                    dest_goals = dest_handle.get_goals(relative_object=dest_handle.target, for_carryable=for_carryable, goal_height_limit=height_limit, target_reference_override=target_reference_override, check_height_clearance=False)
                    slope_restricted = target_reference_override is None and dest_handle.target is not None and (dest_handle.constraint.restricted_on_slope and not interaction.ignore_slope_restrictions)
                        ...
                        if middle_path_pickup and carry_object_at_pool and dest_goal.routing_surface_id.type != routing.SurfaceType.SURFACETYPE_POOL:
                            ...
                            if slope_restricted and dest_goal.routing_surface_id.type == routing.SurfaceType.SURFACETYPE_WORLD and dest_goal.position.y != routing_constants.INVALID_TERRAIN_HEIGHT:
                                ...
                                if height_difference > PostureTuning.CONSTRAINT_HEIGHT_TOLERANCE:
                                    ...
                                    dest_is_valid = False
                                elif required_height_clearance > dest_goal.height_clearance:
                                    ...
                                    if not (in_vehicle and current_posture_target is dest_handle.target):
                                        (result, blocking_obj_id) = dest_handle.target.check_line_of_sight(dest_goal.location.transform, verbose=True, for_carryable=for_carryable, use_standard_ignored_objects=True)
