...
def purchase_picker_response_by_ids(inventory_target:RequiredTargetParam, inventory_source:RequiredTargetParam, currency_type:int, mailman_purchase:bool=False, object_ids_or_definition_ids:bool=False, *ids_and_amounts_and_price, _connection=None):
    total_price = 0
    current_purchased = 0
    objects_to_buy = []
    definition_manager = services.definition_manager()
            ...
            if obj is None:
                ...
                if not to_inventory.player_try_add_object(obj):
                    ...
                    if obj.definition.id not in obj_purchased:
                        obj_purchased[obj.definition.id] = {'price': 0, 'amount': 0}
    ...
    if proto.items is not None:
        ...
        for item_data in proto.items:
            if item_data not in inventory_component:
                ...
                if item is None:
                    pass
                elif item.non_deletable_by_user:
                    pass
                else:
...
def sim_inventory_favorite_multiple(sim_id:int=0, is_add:bool=False, *items, _connection=None):
