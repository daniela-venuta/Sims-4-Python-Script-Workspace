...
class StatisticAddRelationshipMultiple(RelationshipOperationBase):
    ...
    def _apply_relationship_operation(self, relationship_tracker, target_sim_id, **kwargs):
        for track in self._track_selection_strategy.get_tracks(relationship_tracker, target_sim_id):
            relationship_tracker.add_relationship_score(target_sim_id, self._amount, track=track.stat_type, **kwargs)
...
class StatisticSetRelationship(RelationshipOperation):
    ...
    def _apply_relationship_operation(self, relationship_tracker, target_sim_id, **kwargs):
        if not self._add_track_if_not_present:
            ...
            if not services.relationship_service().has_relationship_track(actor_sim_id, target_sim_id, self._track):
                return
        relationship_tracker.set_relationship_score(target_sim_id, self._value, track=self._track.stat_type, **kwargs)
...
class StatisticSetRelationshipMultiple(RelationshipOperationBase):
    ...
    def _apply_relationship_operation(self, relationship_tracker, target_sim_id, **kwargs):
        for track in self._track_selection_strategy.get_tracks(relationship_tracker, target_sim_id):
            relationship_tracker.set_relationship_score(target_sim_id, self._value, track=track.stat_type, **kwargs)
