...
class Sim(HasSimInfoMixin, LockoutMixin, EnvironmentScoreMixin, GameObject):
    ...
    def get_location_on_nearest_surface_below(self):
        if self.posture_state.valid and self.posture.unconstrained and not (self.posture.consider_constrained_for_on_surface_checks or self.active_transition is not None and self.active_transition.source.unconstrained):
    ...
    def _potential_behavior_affordances_gen(self, context, **kwargs):
        ...
        for affordance in self.sim_info.unlock_tracker.get_cached_super_affordances_gen():
            if self._can_show_affordance(shift_held, affordance):
                yield affordance
        if context is not None and self.sim_info.unlock_tracker is not None and self.sim_info.career_tracker is not None:
