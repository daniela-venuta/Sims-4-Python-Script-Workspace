...
class SimInfoManager(DistributableObjectManager):
    ...
    def on_zone_load(self):
        ...
        fw_cleanup_played_sim_ids = set()
        clean_up_father_winter = False
        for sim_info in tuple(self.values()):
            sim_info.on_zone_load()
            if self._clear_father_winter(sim_info, clean_up_father_winter, fw_cleanup_played_sim_ids):
                clean_up_father_winter = True
            elif sim_info.sim_id in self._sims_traveled_to_zone:
    ...
    def _clear_father_winter(self, sim_info, clean_up_father_winter, played_sim_ids):
        if self.FATHER_WINTER_TRAIT is None:
            return False
        ...
        if not household.hidden:
            return False
        if not sim_info.has_trait(self.FATHER_WINTER_TRAIT):
            return False
